...
template<class T>
std::function<void()> ActorWithComponentCreationHandler(GeCore::ECS::Actor* p_parent, std::optional<std::function<void()>> p_onItemClicked)
{
    return Combine(EDITOR_BIND(CreateMonoComponentActor<T>, true, p_parent), p_onItemClicked);
}

std::function<void()> ActorWithModelComponentCreationHandler(GeCore::ECS::Actor* p_parent, const std::string& p_modelName, std::optional<std::function<void()>> p_onItemClicked)
{
    return Combine(EDITOR_BIND(CreateActorWithModel, ":Models\\" + p_modelName + ".fbx", true, p_parent, p_modelName), p_onItemClicked);
}

void GeEditor::Utils::ActorCreationMenu::GenerateActorCreationMenu(GeUI::Widgets::Menu::MenuList& p_menuList, GeCore::ECS::Actor* p_parent, std::optional<std::function<void()>> p_onItemClicked)
{
