...
#include <GeUI/Widgets/Sliders/SliderInt.h>
#include <GeUI/Widgets/Sliders/SliderFloat.h>
...
GeEditor::Panels::MenuBar::MenuBar()
{
	CreateFileMenu();
	CreateBuildMenu();
	CreateWindowMenu();
...
void GeEditor::Panels::MenuBar::HandleShortcuts(float p_deltaTime)
{
	auto& inputManager = *EDITOR_CONTEXT(inputManager);

	if (inputManager.GetKeyState(GeWindow::Inputs::EKey::KEY_LEFT_CONTROL) == GeWindow::Inputs::EKeyState::KEY_DOWN)
	{
		if (inputManager.IsKeyPressed(GeWindow::Inputs::EKey::KEY_N))
			EDITOR_EXEC(LoadEmptyScene());

		if (inputManager.IsKeyPressed(GeWindow::Inputs::EKey::KEY_S))
		{
			if (inputManager.GetKeyState(GeWindow::Inputs::EKey::KEY_LEFT_SHIFT) == GeWindow::Inputs::EKeyState::KEY_UP)
...
void GeEditor::Panels::MenuBar::CreateFileMenu()
{
...
void GeEditor::Panels::MenuBar::CreateBuildMenu()
{
...
void GeEditor::Panels::MenuBar::CreateWindowMenu()
{
...
void GeEditor::Panels::MenuBar::CreateSettingsMenu()
{
	...
	cameraSpeedMenu.CreateWidget<GeUI::Widgets::Sliders::SliderInt>(1, 50, 15, GeUI::Widgets::Sliders::ESliderOrientation::HORIZONTAL, "Scene View").ValueChangedEvent += EDITOR_BIND(SetSceneViewCameraSpeed, std::placeholders::_1);
	cameraSpeedMenu.CreateWidget<GeUI::Widgets::Sliders::SliderInt>(1, 50, 15, GeUI::Widgets::Sliders::ESliderOrientation::HORIZONTAL, "Asset View").ValueChangedEvent += EDITOR_BIND(SetAssetViewCameraSpeed, std::placeholders::_1);
	...
	auto& sceneViewBackgroundPicker = sceneViewBackground.CreateWidget<Selection::ColorEdit>(false, GeUI::Types::Color{ 0.098f, 0.098f, 0.098f });
    ...
    auto& sceneViewGridPicker = sceneViewGrid.CreateWidget<Selection::ColorEdit>(false, GeUI::Types::Color(0.176f, 0.176f, 0.176f));
	...
	{
		EDITOR_PANEL(Panels::SceneView, "Scene View").SetGridColor(GeMaths::FVector3(0.176f, 0.176f, 0.176f));
		sceneViewGridPicker.color = GeUI::Types::Color(0.176f, 0.176f, 0.176f);
	};

	auto& assetViewBackground = viewColors.CreateWidget<MenuList>("Asset View Background");
	auto& assetViewBackgroundPicker = assetViewBackground.CreateWidget<Selection::ColorEdit>(false, GeUI::Types::Color{ 0.098f, 0.098f, 0.098f });
	...
	auto& assetViewGridPicker = assetViewGrid.CreateWidget<Selection::ColorEdit>(false, GeUI::Types::Color(0.176f, 0.176f, 0.176f));
	...
	{
		EDITOR_PANEL(Panels::AssetView, "Asset View").SetGridColor(GeMaths::FVector3(0.176f, 0.176f, 0.176f));
		assetViewGridPicker.color = GeUI::Types::Color(0.176f, 0.176f, 0.176f);
	};

	auto& sceneViewBillboardScaleMenu = settingsMenu.CreateWidget<MenuList>("3D Icons Scales");
	auto& lightBillboardScaleSlider = sceneViewBillboardScaleMenu.CreateWidget<Sliders::SliderInt>(0, 100, static_cast<int>(Settings::EditorSettings::LightBillboardScale * 100.0f), GeUI::Widgets::Sliders::ESliderOrientation::HORIZONTAL, "Lights");
...
void GeEditor::Panels::MenuBar::CreateLayoutMenu() 
{
	auto& layoutMenu = CreateWidget<MenuList>("Layout");
	layoutMenu.CreateWidget<MenuItem>("Reset").ClickedEvent += EDITOR_BIND(ResetLayout);
}

void GeEditor::Panels::MenuBar::CreateHelpMenu()
{
    auto& helpMenu = CreateWidget<MenuList>("Help");
    helpMenu.CreateWidget<MenuItem>("GitHub").ClickedEvent += [] {GeUtils::Utils::SystemCalls::OpenURL("https://github.com/adriengivry/Overload"); };
    helpMenu.CreateWidget<MenuItem>("Tutorials").ClickedEvent += [] {GeUtils::Utils::SystemCalls::OpenURL("https://github.com/adriengivry/Overload/wiki/Tutorials"); };
    helpMenu.CreateWidget<MenuItem>("Scripting API").ClickedEvent += [] {GeUtils::Utils::SystemCalls::OpenURL("https://github.com/adriengivry/Overload/wiki/Scripting-API"); };
    helpMenu.CreateWidget<Visual::Separator>();
    helpMenu.CreateWidget<MenuItem>("Bug Report").ClickedEvent += [] {GeUtils::Utils::SystemCalls::OpenURL("https://github.com/adriengivry/Overload/issues/new?assignees=&labels=Bug&template=bug_report.md&title="); };
    helpMenu.CreateWidget<MenuItem>("Feature Request").ClickedEvent += [] {GeUtils::Utils::SystemCalls::OpenURL("https://github.com/adriengivry/Overload/issues/new?assignees=&labels=Feature&template=feature_request.md&title="); };
    helpMenu.CreateWidget<Visual::Separator>();
    helpMenu.CreateWidget<Texts::Text>("Version: 1.3.0");
}

void GeEditor::Panels::MenuBar::RegisterPanel(const std::string& p_name, GeUI::Panels::PanelWindow& p_panel)
{
	auto& menuItem = m_windowMenu->CreateWidget<MenuItem>(p_name, "", true, true);
	menuItem.ValueChangedEvent += std::bind(&GeUI::Panels::PanelWindow::SetOpened, &p_panel, std::placeholders::_1);

	m_panels.emplace(p_name, std::make_pair(std::ref(p_panel), std::ref(menuItem)));
}

void GeEditor::Panels::MenuBar::UpdateToggleableItems()
{
	for (auto&[name, panel] : m_panels)
		panel.second.get().checked = panel.first.get().IsOpened();
}

void GeEditor::Panels::MenuBar::OpenEveryWindows(bool p_state)
{
	for (auto&[name, panel] : m_panels)
