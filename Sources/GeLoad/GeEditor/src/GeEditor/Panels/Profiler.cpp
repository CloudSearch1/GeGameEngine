...
GeEditor::Panels::Profiler::Profiler
(
...
void GeEditor::Panels::Profiler::Update(float p_deltaTime)
{
	m_timer += p_deltaTime;
...
void GeEditor::Panels::Profiler::Enable(bool p_value, bool p_disableLog)
{
	if (p_value)
	{
		if (!p_disableLog)
...
GeUI::Types::Color GeEditor::Panels::Profiler::CalculateActionColor(double p_percentage) const
{
...
std::string GeEditor::Panels::Profiler::GenerateActionString(GeAnalytics::Profiling::ProfilerReport::Action & p_action)
{
	std::string result;
