...
GeEditor::Panels::SceneView::SceneView
(
	...
	m_sceneManager(EDITOR_CONTEXT(sceneManager))
{
	...
	m_image->AddPlugin<GeUI::Plugins::DDTarget<std::pair<std::string, GeUI::Widgets::Layout::Group*>>>("File").DataReceivedEvent += [this](auto p_data)
	{
		...
		{
		case GeUtils::Utils::PathParser::EFileType::SCENE:	EDITOR_EXEC(LoadSceneFromDisk(path));			break;
		case GeUtils::Utils::PathParser::EFileType::MODEL:	EDITOR_EXEC(CreateActorWithModel(path, true));	break;
		}
	};
}

void GeEditor::Panels::SceneView::Update(float p_deltaTime)
{
	...
	if (IsFocused() && !m_cameraController.IsRightMousePressed())
	{
		...
		{
			m_currentOperation = GeEditor::Core::EGizmoOperation::TRANSLATE;
		}

		if (EDITOR_CONTEXT(inputManager)->IsKeyPressed(EKey::KEY_E))
		{
			m_currentOperation = GeEditor::Core::EGizmoOperation::ROTATE;
		}

		if (EDITOR_CONTEXT(inputManager)->IsKeyPressed(EKey::KEY_R))
		{
			m_currentOperation = GeEditor::Core::EGizmoOperation::SCALE;
		}
	}
}

void GeEditor::Panels::SceneView::_Render_Impl()
{
	PrepareCamera();
...
void GeEditor::Panels::SceneView::RenderScene(uint8_t p_defaultRenderState)
{
	auto& baseRenderer = *EDITOR_CONTEXT(renderer).get();
	auto& currentScene = *m_sceneManager.GetCurrentScene();
	auto& gameView = EDITOR_PANEL(GeEditor::Panels::GameView, "Game View");
...
void GeEditor::Panels::SceneView::RenderSceneForActorPicking()
{
	auto& baseRenderer = *EDITOR_CONTEXT(renderer).get();
...
void GeEditor::Panels::SceneView::HandleActorPicking()
{
	using namespace GeWindow::Inputs;
	...
	if (IsHovered() && !IsResizing())
	{
		...
		m_actorPickingFramebuffer.Bind();
		uint8_t pixel[3];
		EDITOR_CONTEXT(renderer)->ReadPixels(static_cast<int>(mouseX), static_cast<int>(mouseY), 1, 1, GeRendering::Settings::EPixelDataFormat::RGB, GeRendering::Settings::EPixelDataType::UNSIGNED_BYTE, pixel);
		m_actorPickingFramebuffer.Unbind();
		...
		auto direction = m_gizmoOperations.IsPicking() ? m_gizmoOperations.GetDirection() : EDITOR_EXEC(IsAnyActorSelected()) && pixel[0] == 255 && pixel[1] == 255 && pixel[2] >= 252 && pixel[2] <= 254 ? static_cast<GeEditor::Core::GizmoBehaviour::EDirection>(pixel[2] - 252) : std::optional<Core::GizmoBehaviour::EDirection>{};
