...
using namespace GeCore::Global;
using namespace GeCore::ResourceManagement;

GeEditor::Core::Context::Context(const std::string& p_projectPath, const std::string& p_projectName) :
	...
	projectSettings(projectFilePath)
{
	...
	device = std::make_unique<GeWindow::Context::Device>(deviceSettings);
	window = std::make_unique<GeWindow::Window>(*device, windowSettings);
	...
	inputManager = std::make_unique<GeWindow::Inputs::InputManager>(*window);
	window->MakeCurrentContext();

	device->SetVsync(true);

	driver = std::make_unique<GeRendering::Context::Driver>(GeRendering::Settings::DriverSettings{ true });
	renderer = std::make_unique<GeCore::ECS::Renderer>(*driver);
	renderer->SetCapability(GeRendering::Settings::ERenderingCapability::MULTISAMPLE, true);
	shapeDrawer = std::make_unique<GeRendering::Core::ShapeDrawer>(*renderer);

	std::filesystem::create_directories(std::string(getenv("APPDATA")) + "\\OverloadTech\\GeEditor\\");

	uiManager = std::make_unique<GeUI::Core::UIManager>(window->GetGlfwWindow(), GeUI::Styling::EStyle::ALTERNATIVE_DARK);
	...
	uiManager->SetEditorLayoutSaveFilename(std::string(getenv("APPDATA")) + "\\OverloadTech\\GeEditor\\layout.ini");
	uiManager->SetEditorLayoutAutosaveFrequency(60.0f);
	uiManager->EnableEditorLayoutSave(true);
	uiManager->EnableDocking(true);

	if (!std::filesystem::exists(std::string(getenv("APPDATA")) + "\\OverloadTech\\GeEditor\\layout.ini"))
		uiManager->ResetLayout("Config\\layout.ini");

	audioEngine = std::make_unique<GeAudio::Core::AudioEngine>(projectAssetsPath);
	audioPlayer = std::make_unique<GeAudio::Core::AudioPlayer>(*audioEngine);

	editorResources = std::make_unique<GeEditor::Core::EditorResources>(editorAssetsPath);

	physicsEngine = std::make_unique<GePhysics::Core::PhysicsEngine>(GePhysics::Settings::PhysicsSettings{ {0.0f, -9.81f, 0.0f } });

	ServiceLocator::Provide<GePhysics::Core::PhysicsEngine>(*physicsEngine);
	...
	ServiceLocator::Provide<GeWindow::Inputs::InputManager>(*inputManager);
	ServiceLocator::Provide<GeWindow::Window>(*window);
	ServiceLocator::Provide<GeCore::SceneSystem::SceneManager>(sceneManager);
	ServiceLocator::Provide<GeAudio::Core::AudioEngine>(*audioEngine);
	ServiceLocator::Provide<GeAudio::Core::AudioPlayer>(*audioPlayer);

	scriptInterpreter = std::make_unique<GeCore::Scripting::ScriptInterpreter>(projectScriptsPath);

	engineUBO = std::make_unique<GeRendering::Buffers::UniformBuffer>
	(
		...
		sizeof(GeMaths::FMatrix4),
		0, 0,
		GeRendering::Buffers::EAccessSpecifier::STREAM_DRAW
	);

	lightSSBO			= std::make_unique<GeRendering::Buffers::ShaderStorageBuffer>(GeRendering::Buffers::EAccessSpecifier::STREAM_DRAW);
	simulatedLightSSBO	= std::make_unique<GeRendering::Buffers::ShaderStorageBuffer>(GeRendering::Buffers::EAccessSpecifier::STREAM_DRAW); // Used in Asset View

	std::vector<GeMaths::FMatrix4> simulatedLights;

	GeMaths::FTransform simulatedLightTransform;
	simulatedLightTransform.SetLocalRotation(GeMaths::FQuaternion({ 45.f, 180.f, 10.f }));

	GeRendering::Entities::Light simulatedDirectionalLight(simulatedLightTransform, GeRendering::Entities::Light::Type::DIRECTIONAL);
	simulatedDirectionalLight.color = { 1.f, 1.f, 1.f };
	simulatedDirectionalLight.intensity = 1.f;

	GeRendering::Entities::Light simulatedAmbientLight(simulatedLightTransform, GeRendering::Entities::Light::Type::AMBIENT_SPHERE);
	...
	simulatedLightSSBO->SendBlocks<GeMaths::FMatrix4>(simulatedLights.data(), simulatedLights.size() * sizeof(GeMaths::FMatrix4));

	ApplyProjectSettings();
}

GeEditor::Core::Context::~Context()
{
...
void GeEditor::Core::Context::ResetProjectSettings()
{
...
bool GeEditor::Core::Context::IsProjectSettingsIntegrityVerified()
{
	return
...
void GeEditor::Core::Context::ApplyProjectSettings()
{
