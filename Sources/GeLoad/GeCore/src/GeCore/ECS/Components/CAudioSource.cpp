...
GeCore::ECS::Components::CAudioSource::CAudioSource(ECS::Actor& p_owner) :
	AComponent(p_owner),
	m_audioSource(GeCore::Global::ServiceLocator::Get<GeAudio::Core::AudioPlayer>(), owner.transform.GetFTransform())
{
}

std::string GeCore::ECS::Components::CAudioSource::GetName()
{
	return "Audio Source";
}

void GeCore::ECS::Components::CAudioSource::SetSound(GeAudio::Resources::Sound* p_sound)
{
	m_sound = p_sound;
}

void GeCore::ECS::Components::CAudioSource::SetAutoplay(bool p_autoplay)
{
	m_autoPlay = p_autoplay;
}

void GeCore::ECS::Components::CAudioSource::SetVolume(float p_volume)
{
	m_audioSource.SetVolume(p_volume);
}

void GeCore::ECS::Components::CAudioSource::SetPan(float p_pan)
{
	m_audioSource.SetPan(p_pan);
}

void GeCore::ECS::Components::CAudioSource::SetLooped(bool p_looped)
{
	m_audioSource.SetLooped(p_looped);
}

void GeCore::ECS::Components::CAudioSource::SetPitch(float p_pitch)
{
	m_audioSource.SetPitch(p_pitch);
}

void GeCore::ECS::Components::CAudioSource::SetSpatial(bool p_value)
{
	m_audioSource.SetSpatial(p_value);
}

void GeCore::ECS::Components::CAudioSource::SetAttenuationThreshold(float p_distance)
{
	m_audioSource.SetAttenuationThreshold(p_distance);
}

GeAudio::Resources::Sound* GeCore::ECS::Components::CAudioSource::GetSound() const
{
	return m_sound;
}

bool GeCore::ECS::Components::CAudioSource::IsAutoplayed() const
{
	return m_autoPlay;
}

float GeCore::ECS::Components::CAudioSource::GetVolume() const
{
	return m_audioSource.GetVolume();
}

float GeCore::ECS::Components::CAudioSource::GetPan() const
{
	return m_audioSource.GetPan();
}

bool GeCore::ECS::Components::CAudioSource::IsLooped() const
{
	return m_audioSource.IsLooped();
}

float GeCore::ECS::Components::CAudioSource::GetPitch() const
{
	return m_audioSource.GetPitch();
}

bool GeCore::ECS::Components::CAudioSource::IsFinished() const
{
	return m_audioSource.IsFinished();
}

bool GeCore::ECS::Components::CAudioSource::IsSpatial() const
{
	return m_audioSource.IsSpatial();
}

float GeCore::ECS::Components::CAudioSource::GetAttenuationThreshold() const
{
	return m_audioSource.GetAttenuationThreshold();
}

void GeCore::ECS::Components::CAudioSource::Play()
{
	if (owner.IsActive() && m_sound)
		m_audioSource.Play(*m_sound);
}

void GeCore::ECS::Components::CAudioSource::Pause()
{
	if (owner.IsActive())
		m_audioSource.Pause();
}

void GeCore::ECS::Components::CAudioSource::Resume()
{
	if (owner.IsActive())
		m_audioSource.Resume();
}

void GeCore::ECS::Components::CAudioSource::Stop()
{
	if (owner.IsActive())
		m_audioSource.Stop();
}

void GeCore::ECS::Components::CAudioSource::OnSerialize(tinyxml2::XMLDocument& p_doc, tinyxml2::XMLNode* p_node)
{
...
void GeCore::ECS::Components::CAudioSource::OnDeserialize(tinyxml2::XMLDocument& p_doc, tinyxml2::XMLNode* p_node)
{
	using namespace GeCore::Helpers;
...
void GeCore::ECS::Components::CAudioSource::OnInspector(GeUI::Internal::WidgetContainer& p_root)
{
	using namespace GeAudio::Entities;
	using namespace GeCore::Helpers;
	...
	auto& graph = p_root.CreateWidget<GeUI::Widgets::Plots::PlotLines>(std::vector<float>(), -0.1f, 1.1f);

	graph.size.y = 75.0f;

	auto& graphDispacther = graph.AddPlugin<GeUI::Plugins::DataDispatcher<std::vector<float>>>();
	...
	{ 
		...
		GeMaths::FVector3 listenerPosition(0.0f, 0.0f, 0.0f);
		bool playMode = GeCore::Global::ServiceLocator::Get<SceneSystem::SceneManager>().GetCurrentScene()->IsPlaying();
		auto listenerInfo = GeCore::Global::ServiceLocator::Get<GeAudio::Core::AudioEngine>().GetListenerInformation(!playMode);
		if (listenerInfo.has_value())
			listenerPosition = listenerInfo.value().first;

		float distanceToListener = GeMaths::FVector3::Distance(listenerPosition, owner.transform.GetWorldPosition());
...
void GeCore::ECS::Components::CAudioSource::OnEnable()
{
	if (m_autoPlay)
		Play();
}

void GeCore::ECS::Components::CAudioSource::OnDisable()
{
	m_audioSource.Stop();
}
