...
namespace GeCore::ECS
{
	...
	inline T& GeCore::ECS::Actor::AddComponent(Args&& ...p_args)
	{
		...
		if (auto found = GetComponent<T>(); !found)
		{
			...
			{
				reinterpret_cast<GeCore::ECS::Components::AComponent&>(instance).OnAwake();
				reinterpret_cast<GeCore::ECS::Components::AComponent&>(instance).OnEnable();
				reinterpret_cast<GeCore::ECS::Components::AComponent&>(instance).OnStart();
			}
			return instance;
		}
		else
	...
	template<typename T>
	inline bool GeCore::ECS::Actor::RemoveComponent()
	{
		static_assert(std::is_base_of<Components::AComponent, T>::value, "T should derive from AComponent");
