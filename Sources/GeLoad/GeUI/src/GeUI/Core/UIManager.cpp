...
GeUI::Core::UIManager::UIManager(GLFWwindow* p_glfwWindow, Styling::EStyle p_style, const std::string& p_glslVersion)
{
	ImGui::CreateContext();
...
void GeUI::Core::UIManager::ApplyStyle(Styling::EStyle p_style)
{
	ImGuiStyle* style = &ImGui::GetStyle();

	switch (p_style)
	{
	case GeUI::Styling::EStyle::IM_CLASSIC_STYLE:	ImGui::StyleColorsClassic();	break;
	case GeUI::Styling::EStyle::IM_DARK_STYLE:		ImGui::StyleColorsDark();		break;
	case GeUI::Styling::EStyle::IM_LIGHT_STYLE:		ImGui::StyleColorsLight();		break;
	}

	if (p_style == GeUI::Styling::EStyle::DUNE_DARK)
	{
    ...
    else if (p_style == GeUI::Styling::EStyle::ALTERNATIVE_DARK)
    {
        style->WindowPadding = ImVec2(15, 15);
...
bool GeUI::Core::UIManager::LoadFont(const std::string& p_id, const std::string & p_path, float p_fontSize)
{
...
bool GeUI::Core::UIManager::UnloadFont(const std::string & p_id)
{
	if (m_fonts.find(p_id) != m_fonts.end())
	{
		m_fonts.erase(p_id);
...
bool GeUI::Core::UIManager::UseFont(const std::string & p_id)
{
...
void GeUI::Core::UIManager::UseDefaultFont()
{
	ImGui::GetIO().FontDefault = nullptr;
}

void GeUI::Core::UIManager::EnableEditorLayoutSave(bool p_value)
{
	if (p_value)
...
bool GeUI::Core::UIManager::IsEditorLayoutSaveEnabled() const
{
	return ImGui::GetIO().IniFilename != nullptr;
}

void GeUI::Core::UIManager::SetEditorLayoutSaveFilename(const std::string & p_filename)
{
...
void GeUI::Core::UIManager::SetEditorLayoutAutosaveFrequency(float p_frequency)
{
	ImGui::GetIO().IniSavingRate = p_frequency;
}

float GeUI::Core::UIManager::GetEditorLayoutAutosaveFrequency(float p_frequeny)
{
	return ImGui::GetIO().IniSavingRate;
}

void GeUI::Core::UIManager::EnableDocking(bool p_value)
{
...
void GeUI::Core::UIManager::ResetLayout(const std::string& p_config) const
{
    ImGui::LoadIniSettingsFromDisk(p_config.c_str());
}

bool GeUI::Core::UIManager::IsDockingEnabled() const
{
	return m_dockingState;
}

void GeUI::Core::UIManager::SetCanvas(Modules::Canvas& p_canvas)
{
	RemoveCanvas();
...
void GeUI::Core::UIManager::RemoveCanvas()
{
	m_currentCanvas = nullptr;
}

void GeUI::Core::UIManager::Render()
{
	if (m_currentCanvas)
	{
...
void GeUI::Core::UIManager::PushCurrentFont()
{

}

void GeUI::Core::UIManager::PopCurrentFont()
{
