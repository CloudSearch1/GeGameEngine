...
GeWindow::Dialogs::FileDialog::FileDialog(std::function<int(tagOFNA*)> p_callback, const std::string & p_dialogTitle) :
...
void GeWindow::Dialogs::FileDialog::SetInitialDirectory(const std::string & p_initialDirectory)
{
	m_initialDirectory = p_initialDirectory;
}

void GeWindow::Dialogs::FileDialog::Show(EExplorerFlags p_flags)
{
	OPENFILENAME ofn;
	...
	ofn.hwndOwner = NULL;  // 如果您有一个窗口居中，请将其 HANDLE 放在这里
...
bool GeWindow::Dialogs::FileDialog::HasSucceeded() const
{
	return m_succeeded;
}

std::string GeWindow::Dialogs::FileDialog::GetSelectedFileName()
{
	return m_filename;
}

std::string GeWindow::Dialogs::FileDialog::GetSelectedFilePath()
{
	return m_filepath;
}

std::string GeWindow::Dialogs::FileDialog::GetErrorInfo()
{
	return m_error;
}

bool GeWindow::Dialogs::FileDialog::IsFileExisting() const
{
	return std::filesystem::exists(m_filepath);
}

void GeWindow::Dialogs::FileDialog::HandleError()
{
