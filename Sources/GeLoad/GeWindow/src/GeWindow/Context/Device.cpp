...
GeUtils::Eventing::Event<GeWindow::Context::EDeviceError, std::string> GeWindow::Context::Device::ErrorEvent;

GeWindow::Context::Device::Device(const Settings::DeviceSettings& p_deviceSettings)
{
	BindErrorCallback();
...
GeWindow::Context::Device::~Device()
{
	if (m_isAlive)
	{
		DestroyCursors();
...
std::pair<int16_t, int16_t> GeWindow::Context::Device::GetMonitorSize() const
{
...
GLFWcursor * GeWindow::Context::Device::GetCursorInstance(Cursor::ECursorShape p_cursorShape) const
{
	return m_cursors.at(p_cursorShape);
}

bool GeWindow::Context::Device::HasVsync() const
{
	return m_vsync;
}

void GeWindow::Context::Device::SetVsync(bool p_value)
{
	glfwSwapInterval(p_value ? 1 : 0);
	m_vsync = p_value;
}

void GeWindow::Context::Device::PollEvents() const
{
	glfwPollEvents();
}

float GeWindow::Context::Device::GetElapsedTime() const
{
	return static_cast<float>(glfwGetTime());
}

void GeWindow::Context::Device::BindErrorCallback()
{
...
void GeWindow::Context::Device::CreateCursors()
{
...
void GeWindow::Context::Device::DestroyCursors()
{
