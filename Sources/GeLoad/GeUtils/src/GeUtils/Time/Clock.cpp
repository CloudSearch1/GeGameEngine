...
float GeUtils::Time::Clock::GetFramerate()
{
	return 1.0f / (__DELTA_TIME);
}

float GeUtils::Time::Clock::GetDeltaTime()
{
	return __DELTA_TIME * __TIME_SCALE;
}

float GeUtils::Time::Clock::GetDeltaTimeUnscaled()
{
	return __DELTA_TIME;
}

float GeUtils::Time::Clock::GetTimeSinceStart()
{
	return __TIME_SINCE_START;
}

float GeUtils::Time::Clock::GetTimeScale()
{
	return __TIME_SCALE;
}

void GeUtils::Time::Clock::Scale(float p_coeff)
{
	__TIME_SCALE *= p_coeff;
}

void GeUtils::Time::Clock::SetTimeScale(float p_timeScale)
{
	__TIME_SCALE = p_timeScale;
}

void GeUtils::Time::Clock::Initialize()
{
	__DELTA_TIME = 0.0f;
...
void GeUtils::Time::Clock::Update()
{
