...
namespace GeRendering::Entities
{
  ...
  static uint32_t Pack(const GeMaths::FVector3& p_toPack)
  {
  ...
  //计算环境光半径
  static float CalculateAmbientBoxLightRadius(const GeMaths::FVector3& p_position, const GeMaths::FVector3& p_size)
  {
    return GeMaths::FVector3::Distance(p_position, p_position + p_size);
  }

  //计算灯光亮度
  static float CalculateLuminosity(float p_constant, float p_linear, float p_quadratic, float p_intensity, float p_distance)
  {
  ...
  //计算灯光半径
  static float CalculatePointLightRadius(float p_constant, float p_linear, float p_quadratic, float p_intensity)
  {
...
#define TRY_LESS(value, newValue)\
  ...
  Light::Light(GeMaths::FTransform& p_tranform, Type p_type)
    : m_transform(p_tranform), type(static_cast<float>(p_type))
  {

  }

  GeMaths::FMatrix4 Light::GenerateMatrix() const
  {
  ...
  {
    switch (static_cast<Light::Type>(static_cast<int>(type)))
    {
    case Type::POINT:
  ...
  const GeMaths::FTransform& Light::GetTransform() const
  {
    return m_transform;
  }
