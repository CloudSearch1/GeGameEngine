...
GeRendering::Resources::Mesh::Mesh(const std::vector<Geometry::Vertex>& p_vertices, const std::vector<uint32_t>& p_indices, uint32_t p_materialIndex) :
...
void GeRendering::Resources::Mesh::Bind()
{
	m_vertexArray.Bind();
}

void GeRendering::Resources::Mesh::Unbind()
{
	m_vertexArray.Unbind();
}

uint32_t GeRendering::Resources::Mesh::GetVertexCount()
{
	return m_vertexCount;
}

uint32_t GeRendering::Resources::Mesh::GetIndexCount()
{
	return m_indicesCount;
}

uint32_t GeRendering::Resources::Mesh::GetMaterialIndex() const
{
	return m_materialIndex;
}

const GeRendering::Geometry::BoundingSphere& GeRendering::Resources::Mesh::GetBoundingSphere() const
{
...
void GeRendering::Resources::Mesh::ComputeBoundingSphere(const std::vector<Geometry::Vertex>& p_vertices)
{
	...
	if (!p_vertices.empty())
	{
		...
		for (const auto& vertex : p_vertices)
		{
			const auto& position = reinterpret_cast<const GeMaths::FVector3&>(vertex.position);
			m_boundingSphere.radius = std::max(m_boundingSphere.radius, GeMaths::FVector3::Distance(m_boundingSphere.position, position));
		}
	}
}
