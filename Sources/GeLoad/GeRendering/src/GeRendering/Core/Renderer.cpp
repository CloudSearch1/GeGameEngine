...
namespace GeRendering::Core {
  ...
  Renderer::Renderer(Context::Driver& p_driver)
    : m_driver(p_driver), m_state(0)
  {
  }

  void Renderer::SetClearColor(float p_red, float p_green, float p_blue, float p_alpha)
  {
  ...
  void Renderer::Clear(GeRendering::LowRenderer::Camera& p_camera, bool p_colorBuffer, bool p_depthBuffer, bool p_stencilBuffer)
  {
  ...
  void Renderer::SetRasterizationMode(Settings::ERasterizationMode p_rasterizationMode)
  {
    glPolygonMode(GL_FRONT_AND_BACK, static_cast<GLenum>(p_rasterizationMode));
  }

  void Renderer::SetCapability(Settings::ERenderingCapability p_capability, bool p_value)
  {
    (p_value ? glEnable : glDisable)(static_cast<GLenum>(p_capability));
  }

  bool Renderer::GetCapability(Settings::ERenderingCapability p_capability) const
  {
    return glIsEnabled(static_cast<GLenum>(p_capability));
  }

  void Renderer::SetStencilAlgorithm(Settings::EComparaisonAlgorithm p_algorithm, int32_t p_reference, uint32_t p_mask)
  {
    glStencilFunc(static_cast<GLenum>(p_algorithm), p_reference, p_mask);
  }

  void Renderer::SetDepthAlgorithm(Settings::EComparaisonAlgorithm p_algorithm)
  {
  ...
  void Renderer::SetStencilOperations(Settings::EOperation p_stencilFail, Settings::EOperation p_depthFail, Settings::EOperation p_bothPass)
  {
    glStencilOp(static_cast<GLenum>(p_stencilFail), static_cast<GLenum>(p_depthFail), static_cast<GLenum>(p_bothPass));
  }

  void Renderer::SetCullFace(Settings::ECullFace p_cullFace)
  {
  ...
  void Renderer::ReadPixels(uint32_t x, uint32_t y, uint32_t width, uint32_t height, Settings::EPixelDataFormat format, Settings::EPixelDataType type, void* data)
  {
    glReadPixels(x, y, width, height, static_cast<GLenum>(format), static_cast<GLenum>(type), data);
  }

  bool Renderer::GetBool(GLenum p_parameter)
  {
    GLboolean result;
  ...
  bool Renderer::GetBool(GLenum p_parameter, uint32_t p_index)
  {
    GLboolean result;
  ...
  int Renderer::GetInt(GLenum p_parameter)
  {
    GLint result;
  ...
  int Renderer::GetInt(GLenum p_parameter, uint32_t p_index)
  {
    GLint result;
  ...
  float Renderer::GetFloat(GLenum p_parameter)
  {
    GLfloat result;
  ...
  float Renderer::GetFloat(GLenum p_parameter, uint32_t p_index)
  {
    GLfloat result;
  ...
  double Renderer::GetDouble(GLenum p_parameter)
  {
    GLdouble result;
  ...
  double Renderer::GetDouble(GLenum p_parameter, uint32_t p_index)
  {
    GLdouble result;
  ...
  int64_t Renderer::GetInt64(GLenum p_parameter)
  {
    GLint64 result;
  ...
  int64_t Renderer::GetInt64(GLenum p_parameter, uint32_t p_index)
  {
    GLint64 result;
  ...
  std::string Renderer::GetString(GLenum p_parameter)
  {
    const GLubyte* result = glGetString(p_parameter);
    return result ? reinterpret_cast<const char*>(result) : std::string();
  }

  std::string Renderer::GetString(GLenum p_parameter, uint32_t p_index)
  {
  ...
  void Renderer::Draw(Resources::IMesh& p_mesh, Settings::EPrimitiveMode p_primitiveMode, uint32_t p_instances)
  {
    if (p_instances > 0)
    {
  ...
  std::vector<std::reference_wrapper<GeRendering::Resources::Mesh>> Renderer::GetMeshesInFrustum
  (
  ...
  void Renderer::ApplyStateMask(uint8_t p_mask)
  {
    if (p_mask != m_state)
    {
      ...
      if ((p_mask & 0x04) != (m_state & 0x04))	SetCapability(GeRendering::Settings::ERenderingCapability::BLEND, p_mask & 0x04);
      if ((p_mask & 0x08) != (m_state & 0x08))	SetCapability(GeRendering::Settings::ERenderingCapability::CULL_FACE, p_mask & 0x8);
      if ((p_mask & 0x10) != (m_state & 0x10))	SetCapability(GeRendering::Settings::ERenderingCapability::DEPTH_TEST, p_mask & 0x10);
      
      if ((p_mask & 0x08) && ((p_mask & 0x20) != (m_state & 0x20) || (p_mask & 0x40) != (m_state & 0x40)))
      {
        int backBit = p_mask & 0x20;
        int frontBit = p_mask & 0x40;
        SetCullFace(backBit && frontBit ? GeRendering::Settings::ECullFace::FRONT_AND_BACK :
          (backBit ? GeRendering::Settings::ECullFace::BACK : GeRendering::Settings::ECullFace::FRONT));
      }
  ...
  void Renderer::SetState(uint8_t p_state)
  {
    m_state = p_state;
  }
