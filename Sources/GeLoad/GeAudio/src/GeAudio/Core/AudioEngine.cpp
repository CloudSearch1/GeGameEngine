...
GeAudio::Core::AudioEngine::AudioEngine(const std::string & p_workingDirectory) : m_workingDirectory(p_workingDirectory)
{
	m_irrklangEngine = irrklang::createIrrKlangDevice();

	using AudioSourceReceiver	= void(AudioEngine::*)(GeAudio::Entities::AudioSource&);
	using AudioListenerReceiver = void(AudioEngine::*)(GeAudio::Entities::AudioListener&);
...
GeAudio::Core::AudioEngine::~AudioEngine()
{
	m_irrklangEngine->drop();
}

void GeAudio::Core::AudioEngine::Update()
{
	std::for_each(m_audioSources.begin(), m_audioSources.end(), std::mem_fn(&Entities::AudioSource::UpdateTrackedSoundPosition));

	std::optional<std::pair<GeMaths::FVector3, GeMaths::FVector3>> listener = GetListenerInformation();
...
void GeAudio::Core::AudioEngine::Suspend()
{
...
void GeAudio::Core::AudioEngine::Unsuspend()
{
...
bool GeAudio::Core::AudioEngine::IsSuspended() const
{
	return m_suspended;
}

const std::string& GeAudio::Core::AudioEngine::GetWorkingDirectory() const
{
	return m_workingDirectory;
}

irrklang::ISoundEngine * GeAudio::Core::AudioEngine::GetIrrklangEngine() const
{
	return m_irrklangEngine;
}

std::optional<std::pair<GeMaths::FVector3, GeMaths::FVector3>> GeAudio::Core::AudioEngine::GetListenerInformation(bool p_considerDisabled) const
{
...
void GeAudio::Core::AudioEngine::Consider(GeAudio::Entities::AudioSource & p_audioSource)
{
	m_audioSources.push_back(std::ref(p_audioSource));
}

void GeAudio::Core::AudioEngine::Consider(GeAudio::Entities::AudioListener & p_audioListener)
{
	m_audioListeners.push_back(std::ref(p_audioListener));
}

void GeAudio::Core::AudioEngine::Unconsider(GeAudio::Entities::AudioSource & p_audioSource)
{
...
void GeAudio::Core::AudioEngine::Unconsider(GeAudio::Entities::AudioListener & p_audioListener)
{
