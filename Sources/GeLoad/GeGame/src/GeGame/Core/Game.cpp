...
GeGame::Core::Game::Game(Context & p_context) :
	m_context(p_context),
...
GeGame::Core::Game::~Game()
{
	m_context.sceneManager.UnloadCurrentScene();
}

void GeGame::Core::Game::PreUpdate()
{
	#ifdef _DEBUG
...
void GeGame::Core::Game::Update(float p_deltaTime)
{
	m_elapsed += p_deltaTime;

	m_context.engineUBO->SetSubData(m_elapsed, 3 * sizeof(GeMaths::FMatrix4) + sizeof(GeMaths::FVector3));
	...
	if  (m_context.inputManager->IsKeyPressed(GeWindow::Inputs::EKey::KEY_F12))
		m_showDebugInformation = !m_showDebugInformation;

	#ifdef _DEBUG
	if (m_context.inputManager->IsKeyPressed(GeWindow::Inputs::EKey::KEY_R))
		GeRendering::Resources::Loaders::ShaderLoader::Recompile(*m_context.shaderManager[":Shaders\\Standard.glsl"], "Data\\Engine\\Shaders\\Standard.glsl");
	#endif
...
void GeGame::Core::Game::PostUpdate()
{
	#ifdef _DEBUG
