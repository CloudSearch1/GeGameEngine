	...
	sceneManager(projectAssetsPath)
{
	...
	driver = std::make_unique<GeRendering::Context::Driver>(GeRendering::Settings::DriverSettings{ false });
	renderer = std::make_unique<GeCore::ECS::Renderer>(*driver);

	renderer->SetCapability(GeRendering::Settings::ERenderingCapability::MULTISAMPLE, projectSettings.Get<bool>("multisampling"));
	...
	physicsEngine = std::make_unique<GePhysics::Core::PhysicsEngine>(GePhysics::Settings::PhysicsSettings{ {0.0f, projectSettings.Get<float>("gravity"), 0.0f } });
